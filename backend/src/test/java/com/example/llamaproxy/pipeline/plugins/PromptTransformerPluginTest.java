...
public class PromptTransformerPluginTest {
    ...
    public void setup() {
        ...
        when(settingsManager.getSettingsAs(eq("prompt-transformer"), eq(PromptTransformerPlugin.TransformerSettings.class)))
                .thenReturn(settings);
    }

    @Test
    public void testPromptReplace_NoRules() {
        RequestContext context = new RequestContext(HttpMethod.POST, "/v1/chat/completions", new HttpHeaders(), "Hello World");
    ...
    public void testPromptReplace_BasicRegex() {
        settings.promptReplaceRules = List.of(
                new PromptTransformerPlugin.PromptReplaceRule("World", "Universe")
        );
        RequestContext context = new RequestContext(HttpMethod.POST, "/v1/chat/completions", new HttpHeaders(), "Hello World");
        plugin.processRequest(context);
    ...
    public void testPromptReplace_AdvancedRegex() {
        settings.promptReplaceRules = List.of(
                new PromptTransformerPlugin.PromptReplaceRule("\\b[A-Z]{3}\\b", "XXX")
        );
        RequestContext context = new RequestContext(HttpMethod.POST, "/v1/chat/completions", new HttpHeaders(), "My code is ABC and DEF.");
        plugin.processRequest(context);
    ...
    public void testResponseReplace_BasicRegex() {
        settings.responseReplaceRules = List.of(
                new PromptTransformerPlugin.PromptReplaceRule("badword", "****")
        );
