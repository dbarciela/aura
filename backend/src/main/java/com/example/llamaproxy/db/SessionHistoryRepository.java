package com.example.llamaproxy.db;
...
public class SessionHistoryRepository {
    ...
    public void init() {
        ...
        jdbcTemplate.execute(
            "CREATE TRIGGER session_history_ai AFTER INSERT ON session_history BEGIN " +
            "  INSERT INTO session_history_fts(rowid, id, endpoint, status_code, full_payload) " +
            "  VALUES (new.rowid, new.id, new.endpoint, new.status_code, new.full_payload); " +
            "END;"
        );
        
        jdbcTemplate.execute("DROP TRIGGER IF EXISTS session_history_ad");
        jdbcTemplate.execute(
            "CREATE TRIGGER session_history_ad AFTER DELETE ON session_history BEGIN " +
        ...
        jdbcTemplate.execute(
            "CREATE TRIGGER session_history_au AFTER UPDATE ON session_history BEGIN " +
            "  INSERT INTO session_history_fts(session_history_fts, rowid, id, endpoint, status_code, full_payload) " +
            "  VALUES('delete', old.rowid, old.id, old.endpoint, old.status_code, old.full_payload); " +
            "  INSERT INTO session_history_fts(rowid, id, endpoint, status_code, full_payload) " +
            "  VALUES (new.rowid, new.id, new.endpoint, new.status_code, new.full_payload); " +
    ...
    public void save(String id, String endpoint, int statusCode, String fullPayload) {
        jdbcTemplate.update(
            "INSERT INTO session_history (id, endpoint, status_code, full_payload) VALUES (?, ?, ?, ?)",
    ...
    public void updateImprovedTitle(String id, String improvedTitle) {
        jdbcTemplate.update(
            "UPDATE session_history SET improved_title = ? WHERE id = ?",
            improvedTitle, id
        );
    }
    ...
    public List<Map<String, Object>> search(String query) {
        ...
        // Escape the query for FTS MATCH by wrapping in double quotes and escaping inner quotes
        String escapedQuery = "\"" + query.replace("\"", "\"\"") + "\"";
        
        return jdbcTemplate.queryForList(
            "SELECT sh.* FROM session_history sh " +
            "JOIN session_history_fts fts ON sh.rowid = fts.rowid " +
    ...
    public void deleteByIds(List<String> ids) {
        ...
        jdbcTemplate.update(
            String.format("DELETE FROM session_history WHERE id IN (%s)", inSql),
            ids.toArray()
        );
    }
    ...
    public List<Map<String, Object>> findByIds(List<String> ids) {
        ...
        String inSql = String.join(",", java.util.Collections.nCopies(ids.size(), "?"));
        return jdbcTemplate.queryForList(
            String.format("SELECT id, full_payload FROM session_history WHERE id IN (%s)", inSql),
            ids.toArray()
        );
    }
}
