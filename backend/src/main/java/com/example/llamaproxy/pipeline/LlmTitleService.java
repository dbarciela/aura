...
public class LlmTitleService {
    ...
    public String generateTitle(String payload) {
        try {
            // Very naive way to extract first message content without fully parsing unknown structure
            // In a real app we'd parse this correctly, but since we know it's OpenAI format...
            ...
            if (root.has("messages") && root.get("messages").isArray() && root.get("messages").size() > 0) {
                ...
                java.util.regex.Matcher m = java.util.regex.Pattern.compile("<user(?:Request|_input|Input)>([\\s\\S]*?)</user(?:Request|_input|Input)>", java.util.regex.Pattern.CASE_INSENSITIVE).matcher(firstMessage);
                if (m.find()) {
                    ...
                    firstMessage = firstMessage.replaceAll("(?is)<(environment_info|workspace_info|user_information|identity|plugins|messaging|conversation_transcript|artifacts|slash_commands|subagents)>.*?</\\1>", " ");
            ...
            return restClient.post()
                    ...
                    .exchange((clientRequest, clientResponse) -> {
                        if (clientResponse.getStatusCode().is2xxSuccessful()) {
                            ...
                            if (respNode.has("choices") && respNode.get("choices").isArray() && respNode.get("choices").size() > 0) {
