...
public class ManualEditorPlugin implements BufferingPlugin {
	...
	public void processRequest(RequestContext context) {
		...
		if (pluginSettings == null) {
			pluginSettings = new ManualEditorSettings();
		}
		...
		if (!matchesRegex && regexRules != null && !regexRules.isEmpty()) {
			// Only skip if rules exist but none matched. If no rules exist, we intercept
			// EVERYTHING if interceptRequests is true.
	...
	public void processResponse(ResponseContext context) {
		...
		if (!shouldIntercept) {
			...
			if (regexRules != null && !regexRules.isEmpty()) {
				...
				// If no regex rules and we want to intercept responses, then we intercept ALL
				// responses
				shouldIntercept = true;
			}
		}
